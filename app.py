...
from langchain_google_genai.chat_models import ChatGoogleGenerativeAIError # <-- Add this import
...
st.markdown("""
    ...
    .stApp { /* Main app container */
        ...
        color: #FFFFFF; /* Pure white text for dark background */
    }
    ...
    .css-1fv8s86.eqr7zpz4 { /* Target the info box specifically */
        ...
        color: #FFFFFF; /* Lighter text for info boxes */
        border-radius: 5px;
    }
    </style>
...
st.markdown("<h3 style='text-align: center; color: #ADD8E6;'>Your Intelligent Meeting Assistant</h3>", unsafe_allow_html=True)
...
# Added for signup to login redirect
...
def show_auth_page():
    ...
    default_index = 0 if st.session_state.show_login_tab else 1
    tab1, tab2 = st.tabs(["Login", "Sign Up"], default_index=default_index)
    ...
    with tab1:
        ...
        if st.button("Login"):
            ...
            if user_id:
                ...
                st.session_state.show_login_tab = False # Reset after successful login
                st.rerun()
            else:
        ...
        if st.button("Register"): # This button now applies to the modified CSS
            if new_password != confirm_password:
                ...
                if register_user(new_username, new_password):
                    ...
                    st.session_state.show_login_tab = True # Set to show login tab next
                    st.rerun() # Rerun to switch to login tab
                else:
                ...
                if not gemini_api_key:
                    ...
                    def ask_document(question):
                        ...
                        try: # Added try block
                            ...
                            return "An error occurred while generating the answer. Please check the troubleshooting steps above.", [] # Return default values
