...
st.image("meetingmind_banner.png", width='stretch') # Changed use_column_width=True to width='stretch'
...
def show_auth_page():
    ...
    with tab1:
        ...
        if st.button("Login"): # Removed unique key for button in login tab
    ...
    with tab2:
        ...
        if st.button("Register"): # Removed unique key for button in sign up tab
...
if not st.session_state.authenticated:
    ...
    with st.sidebar:
        ...
        if st.button("Logout"): # Removed unique key for logout button
            logout()
        st.markdown("--- ")
    ...
    if st.session_state.pdf_uploaded and st.session_state.vectorstore is not None:
        ...
        with st.container(border=True):
            ...
            if st.button("Get Answer"): # Removed unique key for Get Answer button
                if not groq_api_key:
                    st.error("Groq API Key not found in Streamlit Secrets. Please ensure it's set.")
                    ...
                    def ask_document(question):
                        ...
                        try:
                            ...
                            answer = str(response.content) # Groq returns a string directly
                        except Exception as e: # Catch generic exception for Groq errors
                            st.error(f"**Groq API Error:** {e}")
                            st.error("This often means your Groq API Key is invalid, expired, or you've exceeded your quota.")
                            st.error("1. Please double-check your `GROQ_API_KEY` in Streamlit secrets and ensure it's correct and active.")
                            st.error("2. If the problem persists, check the Groq documentation for API usage limits or errors.")
